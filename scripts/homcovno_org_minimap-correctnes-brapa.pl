...
my $assembly_path = '/jic/scratch/groups/Richard-Morris/glombikm/brassica_hifi_data/bamfiles/fastq/hifiasm/organelle_filter/homcovBrapa_primary_assembly.no_organelles.fa';
my $ref_path = '/jic/scratch/groups/Richard-Morris/glombikm/references/';
my $species = 'homcovBrapa_no_org_map_';

my $list_of_refs = '/jic/scratch/groups/Richard-Morris/glombikm/references/list.txt';
...
SLURM
 ...
 my $tmp_file = "$output_dir/tmp/minimap.$sample";
  ...
  print SLURM "set -e\n";
	...
	print SLURM "minimap2 -x map-hifi -t 20 -c $reference $assembly_path > $output_dir/$species-$sample".".paf\n";
